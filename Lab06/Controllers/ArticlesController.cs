...
public class ArticlesController : Controller
{
    ...
    // GET: /Articles
    public async Task<IActionResult> Index(int? page, int? categoryId, CancellationToken cancellationToken)
    {
        const int pageSize = 5;
        var currentPage = page ?? 1;

        var totalCount = await _articleService.CountAsync(categoryId, cancellationToken);
        var articles = await _articleService.GetPagedAsync(currentPage, pageSize, categoryId, cancellationToken);
        ...
        var categories = (await _categoryService.GetAllAsync(cancellationToken)).OrderBy(c => c.Name).ToList();
